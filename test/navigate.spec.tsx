...
import {navigate, useNavigate, useRoutes, useNavigationPrompt} from '../src'
import {QueryParam, RouteParams} from '../src/types'
...
describe('useNavigate', () => {
    ...
    test('navigate with a basePath', async () => {
        ...
        const {container} = render(
            <App basePath={basePath} newPath={newPath} />,
        )
    ...
    test('navigate without history', async () => {
        ...
        expect(
            (window.history.replaceState as jest.Mock).mock.calls,
        ).toHaveLength(1)
        expect(
            (window.history.replaceState as jest.Mock).mock.calls[0],
        ).toHaveLength(3)
        expect(
            (window.history.replaceState as jest.Mock).mock.calls[0][2],
        ).toEqual(basePath + newPath)
        jest.clearAllMocks()
    })
})
