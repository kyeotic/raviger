...
import {useRoutes, Link, usePath, useQueryParams, Redirect} from '../../.'
import Nav from './Nav'
...
function Filter() {
    ...
    return (
        <Nav>
            ...
            <p>Filter: {type || ''}</p>
            <p>Query: {name || ''}</p>
            <input type="text" placeholder="Enter a query" value={name || ''} onChange={e => setQuery({name: e.target.value}, false)} />
        </Nav>
    )
}
